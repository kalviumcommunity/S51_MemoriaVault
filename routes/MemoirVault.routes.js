...
const MemoirVault = require("../Model/MemoirVault.model")
...
getRouter.get('/getallusers', async (req, res) => {
    try {
        const users = await MemoirVault.find();
...
getRouter.get('/getuser/:id', async (req, res) => {
    try {
        ...
        const user = await MemoirVault.findById(userId);
...
postRouter.post('/adduser', async (req, res) => {
    try {
        ...
        const newUser = await MemoirVault.create({ ID, Name, Password, ImageURLl, VideoURL, DocumentURL });
...
putRouter.patch('/updateuser/:ID', async (req, res) => {
    try {
        ...
        const existingUser = await MemoirVault.findOne({ID: userId });

        if (!existingUser) {
            return res.status(404).json({ message: 'User not found' });
        }

        const updatedUser = await MemoirVault.findOneAndUpdate(
            { ID: userId },
            { $set: updateFields },
            { new: true }
        );
...
deleteRouter.delete('/deleteuser/:ID', async (req, res) => {
    try {
        const userId = req.params.ID;
        const deletedUser = await MemoirVault.findOneAndDelete({userId});
